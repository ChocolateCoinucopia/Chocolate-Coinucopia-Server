...
class ChartPanel {
  ...
  async #build() {
    ...
    Promise.all([this.#preparePanel(), this.#prepareChart(), this.#prepareEventListeners(), this.#prepareMilestones()]).then(success => this.loader('Generate-Time-Series', false)).catch(error => MISC.loadContractError());
  }
  ...
  async #prepareEventListeners() {
    ...
    document.getElementById(this.#name + '-Panel-Undo').addEventListener('click', this.undo.bind(this));
    document.getElementById(this.#name + '-Panel-Save').addEventListener('click', this.capture.bind(this));
  }
  ...
  generateTimeSeries() {
    ...
    this.#generateTimeSeriesCurve({ Transaction: document.getElementById(this.#name + '-Panel-Transaction').value,
        					                  ...
        					                  Color: document.getElementById(this.#name + '-Panel-Color').value
    });
  }
