...
class Calculator {
	...
	#generateHeaderAnchor(text, horizon = 0) {
		...
		e.appendChild(this.#generateH4(text, horizon));

		return e;
	}

	#generateH4(text, horizon = 0) {
	...
	#generateCellAnchor(text, period = 0, target = null) {
		...
		e.addEventListener('touchstart', this.#isEditable(period, target) ? this.#edit.bind(this, e) : () => MISC.copy(e));
