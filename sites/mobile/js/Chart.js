...
class Chart {
  ...
  save() {
    ...
    e.id = this.#name + '-ScreenShot';

    if(this.chart.Interface) {
  
      this.chart.Chart.Context.drawImage(this.chart.Interface.Canvas, 0, 0);

      e.src = this.chart.Chart.Canvas.toDataURL('image/png');

      this.draw();

      document.getElementById(this.#name + '-Graph').style.display = 'none';
      document.getElementById(this.#name + '-Panel').style.display = 'none';
  ...
  plot() {
    ...
    if(document.getElementById(this.#name + '-ScreenShot')) document.getElementById(this.#name + '-ScreenShot').remove();
    if(document.getElementById(this.#name + '-Panel')) document.getElementById(this.#name + '-Panel').style.display = "";
    if(document.getElementById(this.#name + '-Graph')) document.getElementById(this.#name + '-Graph').style.display = "";
  }
  ...
  #generatePrimaryInterface() {
    ...
    this.chart['Interface']['GUI'] = {x: 0, Lock: false, Touch: 'MOVE', Marks: []};
  ...
  #generatePrimaryFrame() {
    ...
    let p = document.getElementById(this.#name + '-Palette');
    let height = this.chart.Chart.Canvas.height - (p ? p.getBoundingClientRect().height : 0);
    
    this.chart['Frame'] = {};
    
    this.chart.Frame['w'] = Math.round(0.8*this.chart.Chart.Canvas.width);
    this.chart.Frame['h'] = Math.round(0.8*height);
    this.chart.Frame['x'] = Math.round(0.1*this.chart.Chart.Canvas.width);
    this.chart.Frame['y'] = Math.round(0.1*height);
  ...
  #generateSecondaryFrame(secondary) {
    ...
    if(this.chart.Coins.length == 0 && this.chart.Secondary.length == 0) {
      ...
      frame['h'] = Math.round(0.5*this.chart.Frame.h);
      frame['x'] = this.chart.Frame.x;
      frame['y'] = this.chart.Chart.Canvas.height + this.chart.Frame.Padding.y - (p ? p.getBoundingClientRect().height : 0);
      frame['Padding'] = this.chart.Frame.Padding;
  ...
  #drawGridY(chart, index, side = 'L') {
    ...
    chart.Chart.Context.fillText(chart.Grid.y[side].Ticks[index].Label, Math.round(side == 'L' ? chart.Frame.x - 0.1*chart.Frame.Padding.x : chart.Frame.x + chart.Frame.w + 0.1*chart.Frame.Padding.x), this.#getY(chart, chart.Grid.y[side].Ticks[index].y, side));
  }
  ...
  #generateGUIEventListeners() {
    ...
    this.chart.Interface.Canvas.addEventListener('touchmove', this.#touchMoveHandler.bind(this));
    ...
    this.chart.Interface.Canvas.addEventListener('touchstart', this.#touchStartHandler.bind(this));
    this.chart.Interface.Canvas.addEventListener('touchend', this.#touchEndHandler.bind(this));
    this.chart.Interface.Canvas.addEventListener('dblclick', this.#dblClickHandler.bind(this));
  }

  #touchMoveHandler(e) {

    if(!this.#singleTouch(e)) return true;

    let x = (e.touches[0] || e.changedTouches[0]).pageX;
    let y = (e.touches[0] || e.changedTouches[0]).pageY;
    
    this.chart.Interface.Mouse.x = Math.round(x*(this.chart.Interface.Canvas.width/this.chart.Interface.Canvas.clientWidth));
    this.chart.Interface.Mouse.y = Math.round((y - document.getElementById(this.#name + '-Panel').clientHeight)*(this.chart.Interface.Canvas.height/this.chart.Interface.Canvas.clientHeight));
    
    if(this.chart.Interface.Mouse.Focus == null) this.chart.Interface.Mouse.Focus = y > document.getElementById('Chart-Panel').getBoundingClientRect().y + document.getElementById('Chart-Panel').getBoundingClientRect().height;

    if(this.chart.Interface.GUI.Touch == 'DRAW') {
  ...
  #touchStartHandler(e) {
    ...
    let type = 'Line';
    let color = document.getElementById(this.#name + '-Palette-Pen').value;

    this.#touchMoveHandler(e);
    
    this.chart.Interface.Mouse.Active = true;

    if(this.chart.Interface.GUI.Touch == 'DRAW') this.#generateNewMark(type, color);
  }
  ...
  move() {
    ...
    this.chart.Interface.GUI.Touch = 'MOVE';
  }

  doodle() {

    this.chart.Interface.GUI.Touch = 'DRAW';
  }
  ...
  #checkLegendMarkerIconsFire() {
    ...
    if(m) {
      ...
      if(m.Secondary) {
        ...
        this.chart.Secondary[m.Secondary].Coins[m.Index].Color = document.getElementById(this.#name + '-Palette-Color').value;
        ...
        this.chart.Coins[m.Index].Color = document.getElementById(this.#name + '-Palette-Color').value;
  ...
  #drawDetailY(chart, index, x) {
    ...
    chart.Interface.Context.fillText(chart.Coins[index].Data[chart.Coins[index].Index].y.toFixed(18).wei().coin(chart.Coins[index].Units, chart.Grid.y[chart.Coins[index].Axis].Precision), chart.Coins[index].Side == 'L' ? Math.round(0.9*chart.Frame.x) : Math.round(1.1*chart.Frame.x) + chart.Frame.w, y);
 ...
 #scaleX(chart) {
  ...
  align(reset = false) {
    ...
    this.chart.Chart.Canvas.height = (reset ? window.innerHeight - (p ? p.getBoundingClientRect().y + p.getBoundingClientRect().height : 0) : this.chart.Chart.Canvas.clientHeight) - window.scrollY;
    ...
    document.body.style.backgroundSize = '100% ' + (window.scrollY + (p ? p.getBoundingClientRect().y + p.getBoundingClientRect().height : 0) + this.chart.Chart.Canvas.height) + 'px';
  }
